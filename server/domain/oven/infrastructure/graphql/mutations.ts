  ...
  t.field({
    ...
    description: [
      ...
      'Answers `OVEN_UNAVAILABLE` when the account has no oven, `NO_OVEN_PROFILE` when the ' +
        ...
        'screen, `OVEN_BUSY` when it is already cooking, `PROGRAM_UNSUPPORTED` when this oven ' +
        'has no such heating function, and `ASSISTED_NOT_STARTABLE` when the version bakes on one ' +
        'of the oven’s own programmes — the appliance reports those but refuses them as a ' +
        'command, so such a cooking starts from its own panel.',
      '',
      '```graphql',
    ...
    resolve: async (_root, { recipeId, version }, { userId }) => {
      ...
      return match(result)
        ...
        .with('assisted-not-startable', domainError)
