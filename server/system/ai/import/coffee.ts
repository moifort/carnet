...
- parameters: beans (name as the bag spells it, country, producer, roast date, dose), water (what the water is — "Robinet (dureté 3/5)", "Volvic" —, the TOTAL amount, the temperature), extraction (grind, TOTAL brew time, what lands in the cup), milk (null unless the drink has some, written as its type then its brand: "Avoine Oatly", "Vache entier Grandlait"), gear (machine and grinder, brand and model). Never return the machine profile — the preset saved on the machine ("Sera Modern Arc", "Dark roast") is the cook's to name, and the app asks them for it.
