...
- parameters: beans (name as the bag spells it, country, producer, roast date, dose), water (what the water is — "Robinet (dureté 3/5)", "Volvic" —, the TOTAL amount, the temperature), extraction (grind, TOTAL brew time, what lands in the cup), milk (null unless the drink has some, written as its type then its brand: "Avoine Oatly", "Vache entier Grandlait"), gear (machine and grinder, brand and model, plus the profile the machine runs when the source names one — a saved profile like "Sera Modern Arc" or "Dark roast", never one you invent).
- Use null for anything the source does not state. ONE exception: a value ENTIRELY determined by another one you actually read may be computed — the total water from the dose at the method's ratio (about 1:2 on an espresso, 1:16 on a pour-over or a filter machine, 1:12 on an immersion), or the dose from the water the same way. Everything else — the name of the coffee, its country, its producer, its roast date, the grind, the temperature, the machine, the grinder, the machine profile — is NEVER guessed: a missing value is information, an invented one is a lie the cook will brew against.
