...
- **Three recipe types** (`RECIPE_TYPE_VALUES`): `dish`, `thermomix`, `coffee`. `content` is a discriminated union (`kind` mirrors the type); a coffee also carries `Recipe.method` (`BREW_METHOD_VALUES`), aggregate-level like `category`, present iff `type === 'coffee'`, and the axis the app's Café tab reads on. The AI changes **one variable per coffee iteration** — see [docs/business-rules.md](docs/business-rules.md#recipe-types-and-version-content).
